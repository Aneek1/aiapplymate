...
const Landing = () => {
  ...
  return (
    <div className={`min-h-screen ${dark ? 'bg-slate-900' : 'bg-white'}`}>
      {/* Navigation */}
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? dark ? 'bg-slate-900/95 backdrop-blur-md shadow-sm border-b border-slate-700' : 'bg-white/95 backdrop-blur-md shadow-sm border-b border-slate-100' : 'bg-transparent'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-8">
              <a href="#how-it-works" className={`text-sm font-medium transition-colors ${dark ? 'text-slate-400 hover:text-primary' : 'text-slate-600 hover:text-primary'}`}>How It Works</a>
              <a href="#demo" className={`text-sm font-medium transition-colors ${dark ? 'text-slate-400 hover:text-primary' : 'text-slate-600 hover:text-primary'}`}>Demo</a>
              <a href="#features" className={`text-sm font-medium transition-colors ${dark ? 'text-slate-400 hover:text-primary' : 'text-slate-600 hover:text-primary'}`}>Features</a>
              <a href="#testimonials" className={`text-sm font-medium transition-colors ${dark ? 'text-slate-400 hover:text-primary' : 'text-slate-600 hover:text-primary'}`}>Testimonials</a>
              <a href="#faq" className={`text-sm font-medium transition-colors ${dark ? 'text-slate-400 hover:text-primary' : 'text-slate-600 hover:text-primary'}`}>FAQ</a>
            </div>
            <div className="flex items-center gap-3">
              <ThemeCustomizer />
              <a href="https://github.com/Aneek1/aiapplymate" target="_blank" rel="noopener noreferrer" className={`transition-colors ${dark ? 'text-slate-400 hover:text-white' : 'text-slate-500 hover:text-slate-900'}`}>
                <Github className="w-5 h-5" />
              </a>
              <Link to="/app">
                <Button className="bg-primary hover:opacity-90 text-white font-bold rounded-xl px-5 py-2 shadow-lg">
                  Open App
      ...
      <section className="relative pt-32 pb-20 overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className={`absolute top-20 left-10 w-72 h-72 rounded-full blur-3xl ${dark ? 'bg-primary/10' : 'bg-emerald-100/50'}`} />
          <div className={`absolute top-40 right-20 w-96 h-96 rounded-full blur-3xl ${dark ? 'bg-blue-900/20' : 'bg-blue-100/40'}`} />
          <div className={`absolute bottom-10 left-1/3 w-80 h-80 rounded-full blur-3xl ${dark ? 'bg-violet-900/20' : 'bg-violet-100/40'}`} />
        </div>

        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-4xl mx-auto">
            <div className={`inline-flex items-center gap-2 px-4 py-2 border rounded-full mb-8 ${dark ? 'bg-primary/10 border-primary/20' : 'bg-emerald-50 border-emerald-100'}`}>
              <Code2 className="w-4 h-4 text-primary" />
              <span className={`text-sm font-semibold ${dark ? 'text-primary' : 'text-emerald-700'}`}>100% Open Source & Free</span>
              <span className={`text-sm ${dark ? 'text-slate-400' : 'text-emerald-500'}`}>No account required</span>
            </div>

            <h1 className={`text-5xl sm:text-6xl lg:text-7xl font-extrabold leading-[1.1] tracking-tight mb-6 ${dark ? 'text-white' : 'text-slate-900'}`}>
              Tailor Your Resume{' '}
              <span className="text-primary">For Every Job</span>{' '}
              In Seconds
            </h1>

            <p className={`text-xl sm:text-2xl max-w-3xl mx-auto leading-relaxed mb-10 ${dark ? 'text-slate-400' : 'text-slate-600'}`}>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16">
              <Link to="/app">
                <Button className="bg-primary hover:opacity-90 text-white text-lg px-10 py-6 rounded-xl font-bold shadow-xl transition-all hover:shadow-2xl hover:-translate-y-0.5">
                  Start Tailoring — It's Free
                  <ArrowRight className="w-5 h-5 ml-2" />
                </Button>
              </Link>
              <a href="#how-it-works" className={`font-semibold flex items-center gap-2 transition-colors ${dark ? 'text-slate-400 hover:text-primary' : 'text-slate-600 hover:text-primary'}`}>
                See how it works
            ...
            <div className="relative max-w-5xl mx-auto">
              <div className={`rounded-2xl border shadow-2xl p-1 overflow-hidden ${dark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                <div className={`rounded-xl p-6 sm:p-8 ${dark ? 'bg-slate-800/50' : 'bg-slate-50'}`}>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className={`rounded-xl p-5 border shadow-sm ${dark ? 'bg-slate-700 border-slate-600' : 'bg-white border-slate-100'}`}>
                      <div className="flex items-center gap-2 mb-3">
                        <Upload className="w-5 h-5 text-primary" />
                        <span className={`font-bold text-sm ${dark ? 'text-white' : 'text-slate-900'}`}>Your Resume</span>
                      </div>
                      <div className="space-y-2">
                        <div className={`h-2 rounded w-full ${dark ? 'bg-slate-600' : 'bg-slate-200'}`} />
                        <div className={`h-2 rounded w-4/5 ${dark ? 'bg-slate-600' : 'bg-slate-200'}`} />
                        <div className={`h-2 rounded w-3/4 ${dark ? 'bg-slate-600' : 'bg-slate-200'}`} />
                        <div className={`h-2 rounded w-full mt-3 ${dark ? 'bg-slate-600/50' : 'bg-slate-100'}`} />
                        <div className={`h-2 rounded w-5/6 ${dark ? 'bg-slate-600/50' : 'bg-slate-100'}`} />
                      </div>
                    </div>
                    <div className="flex items-center justify-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <span className="text-xs font-bold text-primary uppercase tracking-wider">AI Tailoring</span>
                        <div className="hidden md:flex flex-col items-center gap-1">
                          <div className="w-px h-4 bg-primary/40" />
                          <ArrowRight className="w-4 h-4 text-primary/60 rotate-90 md:rotate-0" />
                        </div>
                      </div>
                    </div>
                    <div className={`rounded-xl p-5 border-2 shadow-sm ${dark ? 'bg-slate-700 border-primary/30' : 'bg-white border-primary/30'}`}>
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          <CheckCircle className="w-5 h-5 text-primary" />
                          <span className={`font-bold text-sm ${dark ? 'text-white' : 'text-slate-900'}`}>Tailored Output</span>
                        </div>
                        <span className={`px-2 py-1 text-xs font-bold rounded-full ${dark ? 'bg-primary/15 text-primary' : 'bg-primary/10 text-primary'}`}>92% ATS</span>
                      </div>
                      <div className="space-y-2">
                        <div className={`h-2 rounded w-full ${dark ? 'bg-primary/30' : 'bg-primary/20'}`} />
                        <div className={`h-2 rounded w-5/6 ${dark ? 'bg-primary/30' : 'bg-primary/20'}`} />
                        <div className={`h-2 rounded w-4/5 ${dark ? 'bg-primary/20' : 'bg-primary/10'}`} />
                        <div className="flex gap-1 mt-3">
                          <span className={`px-2 py-0.5 text-[10px] font-bold rounded ${dark ? 'bg-primary/10 text-primary' : 'bg-primary/5 text-primary'}`}>React</span>
                          <span className={`px-2 py-0.5 text-[10px] font-bold rounded ${dark ? 'bg-primary/10 text-primary' : 'bg-primary/5 text-primary'}`}>TypeScript</span>
                          <span className={`px-2 py-0.5 text-[10px] font-bold rounded ${dark ? 'bg-primary/10 text-primary' : 'bg-primary/5 text-primary'}`}>Node.js</span>
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      <section
        ...
        className={`py-24 ${dark ? 'bg-slate-800/50' : 'bg-slate-50'}`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`text-center max-w-3xl mx-auto mb-16 transition-all duration-700 ${isVisible('how-it-works') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <div className={`inline-flex items-center gap-2 px-4 py-2 border rounded-full mb-6 ${dark ? 'bg-primary/10 border-primary/20' : 'bg-emerald-50 border-emerald-100'}`}>
              <Zap className="w-4 h-4 text-primary" />
              <span className={`text-sm font-semibold ${dark ? 'text-primary' : 'text-emerald-700'}`}>Simple 4-Step Process</span>
            </div>
            <h2 className={`text-3xl sm:text-4xl font-extrabold mb-4 tracking-tight ${dark ? 'text-white' : 'text-slate-900'}`}>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {steps.map((item, i) => (
              <div
                key={item.step}
                className={`relative rounded-2xl p-6 border shadow-sm hover:shadow-lg transition-all duration-500 hover:-translate-y-1 ${dark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'} ${isVisible('how-it-works') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                style={{ transitionDelay: `${i * 150}ms` }}
              >
                <div className={`text-5xl font-black absolute top-4 right-4 ${dark ? 'text-slate-700' : 'text-slate-100'}`}>{item.step}</div>
                ...
                <p className={`text-sm leading-relaxed ${dark ? 'text-slate-400' : 'text-slate-600'}`}>{item.description}</p>
              </div>
            ))}
          </div>
          ...
          <div className="text-center mt-12">
            <Link to="/app">
              <Button className="bg-primary hover:opacity-90 text-white font-bold rounded-xl px-8 py-4 shadow-lg">
                Try It Now
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`text-center max-w-3xl mx-auto mb-12 transition-all duration-700 ${isVisible('demo') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <div className={`inline-flex items-center gap-2 px-4 py-2 border rounded-full mb-6 ${dark ? 'bg-primary/10 border-primary/20' : 'bg-emerald-50 border-emerald-100'}`}>
              ...
              <span className={`text-sm font-semibold ${dark ? 'text-primary' : 'text-emerald-700'}`}>See It In Action</span>
            </div>
            <h2 className={`text-3xl sm:text-4xl font-extrabold mb-4 tracking-tight ${dark ? 'text-white' : 'text-slate-900'}`}>
              Watch How It Works
            </h2>
          ...
          <div className={`max-w-5xl mx-auto transition-all duration-700 ${isVisible('demo') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 ${
                    activeDemo === i
                      ...
                      : dark ? 'bg-slate-700 text-slate-400 hover:bg-slate-600' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                  }`}
                >
                  <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                    activeDemo === i ? 'bg-white/20 text-white' : dark ? 'bg-slate-600 text-slate-400' : 'bg-slate-200 text-slate-500'
                  }`}>
                    {i + 1}
                  </span>
                  ...
                  <div key={i} className="flex-1 h-1 rounded-full overflow-hidden bg-slate-700">
                    <div
                      className={`h-full rounded-full transition-all ${
                        activeDemo === i ? 'bg-primary animate-[grow_4s_linear]' : i < activeDemo ? 'bg-primary w-full' : 'bg-transparent w-0'
                      }`}
            ...
            <div className="text-center mt-6">
              ...
              <p className={`text-sm mt-1 max-w-lg mx-auto ${dark ? 'text-slate-400' : 'text-slate-500'}`}>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`text-center max-w-3xl mx-auto mb-16 transition-all duration-700 ${isVisible('features') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <div className={`inline-flex items-center gap-2 px-4 py-2 border rounded-full mb-6 ${dark ? 'bg-blue-900/20 border-blue-800/30' : 'bg-blue-50 border-blue-100'}`}>
              <BarChart3 className={`w-4 h-4 ${dark ? 'text-blue-400' : 'text-blue-600'}`} />
              <span className={`text-sm font-semibold ${dark ? 'text-blue-400' : 'text-blue-700'}`}>What You Get</span>
            </div>
            <h2 className={`text-3xl sm:text-4xl font-extrabold mb-4 tracking-tight ${dark ? 'text-white' : 'text-slate-900'}`}>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {features.map((feature, i) => (
              <div
                key={feature.title}
                className={`group rounded-2xl p-6 border shadow-sm hover:shadow-lg transition-all duration-500 hover:-translate-y-1 ${dark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'} ${isVisible('features') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                style={{ transitionDelay: `${i * 100}ms` }}
              >
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-5 group-hover:bg-primary transition-colors duration-300 ${dark ? 'bg-primary/15' : 'bg-primary/10'}`}>
                  <feature.icon className="w-6 h-6 text-primary group-hover:text-white transition-colors duration-300" />
                </div>
                <h3 className={`text-lg font-bold mb-2 ${dark ? 'text-white' : 'text-slate-900'}`}>{feature.title}</h3>
                <p className={`text-sm leading-relaxed ${dark ? 'text-slate-400' : 'text-slate-600'}`}>{feature.description}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <a href="https://github.com/Aneek1/aiapplymate" target="_blank" rel="noopener noreferrer">
                <Button className="bg-white text-slate-900 hover:bg-white/90 font-bold rounded-xl px-8 py-4 shadow-lg">
                  <Github className="w-5 h-5 mr-2" />
                  View on GitHub
                </Button>
              </a>
              <Link to="/app">
                <Button className="bg-white/20 hover:bg-white/30 text-white font-bold rounded-xl px-8 py-4 border border-white/30">
                  Launch App
                  <ArrowRight className="w-4 h-4 ml-2" />
                </Button>
              </Link>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`text-center max-w-3xl mx-auto mb-16 transition-all duration-700 ${isVisible('testimonials') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <div className={`inline-flex items-center gap-2 px-4 py-2 border rounded-full mb-6 ${dark ? 'bg-violet-900/20 border-violet-800/30' : 'bg-violet-50 border-violet-100'}`}>
              <Star className={`w-4 h-4 ${dark ? 'text-violet-400' : 'text-violet-600'}`} />
              <span className={`text-sm font-semibold ${dark ? 'text-violet-400' : 'text-violet-700'}`}>What People Say</span>
            </div>
            <h2 className={`text-3xl sm:text-4xl font-extrabold mb-4 tracking-tight ${dark ? 'text-white' : 'text-slate-900'}`}>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {testimonials.map((t, i) => (
              <div
                key={t.name}
                className={`rounded-2xl p-6 border shadow-sm transition-all duration-500 ${dark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'} ${isVisible('testimonials') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                style={{ transitionDelay: `${i * 150}ms` }}
              >
                ...
                <p className={`leading-relaxed mb-6 ${dark ? 'text-slate-300' : 'text-slate-700'}`}>"{t.text}"</p>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className={`text-sm ${dark ? 'text-slate-400' : 'text-slate-500'}`}>{t.role}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      >
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`text-center mb-16 transition-all duration-700 ${isVisible('faq') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <div className={`inline-flex items-center gap-2 px-4 py-2 border rounded-full mb-6 ${dark ? 'bg-amber-900/20 border-amber-800/30' : 'bg-amber-50 border-amber-100'}`}>
              <MessageCircle className={`w-4 h-4 ${dark ? 'text-amber-400' : 'text-amber-600'}`} />
              <span className={`text-sm font-semibold ${dark ? 'text-amber-400' : 'text-amber-700'}`}>FAQ</span>
            </div>
            <h2 className={`text-3xl sm:text-4xl font-extrabold mb-4 tracking-tight ${dark ? 'text-white' : 'text-slate-900'}`}>
          ...
          <div className="space-y-3">
            {faqs.map((faq, i) => (
              <div
                key={i}
                className={`border rounded-xl overflow-hidden transition-all duration-300 ${dark ? (openFaq === i ? 'bg-slate-800 border-primary/30 shadow-md' : 'bg-slate-800 border-slate-700') : (openFaq === i ? 'bg-white border-primary/30 shadow-md' : 'bg-white border-slate-200')}`}
              >
                <button
                  className="w-full px-6 py-4 flex items-center justify-between text-left"
                  onClick={() => setOpenFaq(openFaq === i ? null : i)}
                >
                  <span className={`font-semibold pr-4 ${dark ? 'text-white' : 'text-slate-900'}`}>{faq.q}</span>
                ...
                <div className={`overflow-hidden transition-all duration-300 ${openFaq === i ? 'max-h-96' : 'max-h-0'}`}>
                  <div className="px-6 pb-4">
                    <p className={`leading-relaxed ${dark ? 'text-slate-400' : 'text-slate-600'}`}>{faq.a}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* Final CTA */}
      <section className={`py-20 ${dark ? 'bg-slate-950' : 'bg-slate-900'}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-lg font-extrabold text-white">AI<span className="text-primary">ApplyMate</span></span>
            </div>
